...
#minhas coisas
trainingFile = open("rawTrainingData - 250 copy.csv", "r")
...
#samples = generateDataSamples.generateDataSamples()
#dataSamples = samples.getData()
#training_data = dataSamples[0]
...
def info_gain(left, right, current_uncertainty):
    ...
    resto = p * gini(left) - (1 - p) * gini(right)
    hey = current_uncertainty - resto
    return hey
...
if __name__ == '__main__':
    ...
    #testingFile  = open("decision-trees\\bandeirasTest.csv", "r")
    """ with open("decision-trees\\randomTestingData - CSV.csv", "r") as csv_file:
        csv_readerTest = csv.reader(csv_file, delimiter=";", quoting = csv.QUOTE_NONE, quotechar="\"", dialect='excel')
        rows = [[float(row[0]), float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5]), float(row[6]), float(row[7]), float(row[8]), row[9]] for row in csv_readerTest if row]
        testSamples = []
        for row in rows:
        ...
        #testing_data = testSamples

    testing_data = dataSamples[1]
